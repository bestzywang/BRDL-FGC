r"""Exp 3:
- The codes are used to plot Fig. 3 in the paper.
"""
...
if not args.plot:
    ...
    # Compute mean accuracy for each attack and aggregator
    average_results = last_100_iterations.groupby(['ATK', 'Aggregator', 'f', 'seed'])['Accuracy (%)'].mean().reset_index()
    
    # Compute accuracy standard deviation for each attack and aggregator
    std_results = last_100_iterations.groupby(['ATK', 'Aggregator', 'f', 'seed'])['Accuracy (%)'].std().reset_index()
    ...
    for ax, attack in zip(axes, attacks):
        ...
        for agg in agg_order:
            if agg in attack_data['Aggregator'].values:
                ...
                avg_accuracy = agg_data.groupby('f')['Accuracy (%)'].mean().reset_index()
                std_accuracy = agg_data.groupby('f')['Accuracy (%)'].std().reset_index()
                
                # Use color palette
                color = palette.get(agg, "black")  # Default color is black
                
                # Plot mean curve
                ax.plot(avg_accuracy['f'], avg_accuracy['Accuracy (%)'], marker='o', label=agg, color=color)
                
                # Fill standard deviation area
                # ax.fill_between(avg_accuracy['f'], (avg_accuracy['Accuracy (%)'] - std_accuracy['Accuracy (%)']),
                #                 (avg_accuracy['Accuracy (%)'] + std_accuracy['Accuracy (%)']), color=color, alpha=0.2)
    ...
    plt.xlabel(r'Number of Byzantine workers $f$')
    plt.tight_layout()
    plt.savefig(OUT_DIR + "accuracy_f.pdf", bbox_inches="tight", dpi=720)
    plt.show()