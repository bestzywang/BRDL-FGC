r"""Exp 3:
- The codes are used to plot Fig. 8 (b) in the paper.
"""
...
if not args.plot:
    ...
    stats_results = last_100_iterations.groupby(['Attack', 'Aggregator', 'niid', 's'])['Accuracy (%)'].agg(['mean', 'min', 'max']).reset_index()
    ...
    g.set_axis_labels(r"Degree of heterogeneity $\rho$", "Accuracy (%)", fontsize=16)
    ...
    # Set x and y axes
    g.set(xticks=[0.4, 0.5, 0.6], ylim=(80, 100))
    ...
    plt.tight_layout()
    plt.savefig(OUT_DIR + "all_attacks_comparison0.pdf", bbox_inches="tight", dpi=720)
    plt.show()
